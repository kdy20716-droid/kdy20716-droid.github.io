...
const randomPickOverlayEl = document.getElementById("random-pick-overlay"); // ✨ 추가
...
// --- Custom Modal UI ---
function showCustomModal({ type = 'alert', title = '알림', message = '', defaultValue = '', onConfirm, onCancel, extraBtn }) {
  // 배경 오버레이 생성
  ...
  btnContainer.style.flexWrap = 'wrap'; // 버튼이 많아질 경우 줄바꿈 허용
  ...
  if (extraBtn) {
    ...
    extraButton.textContent = extraBtn.label;
    extraButton.style.padding = '10px 20px';
    extraButton.style.border = 'none';
    extraButton.style.backgroundColor = extraBtn.color || '#f39c12';
    extraButton.style.color = '#fff';
    extraButton.style.borderRadius = '4px';
    extraButton.style.cursor = 'pointer';
    extraButton.style.fontWeight = 'bold';
    extraButton.onclick = () => {
      document.body.removeChild(overlay);
      if (extraBtn.onClick) extraBtn.onClick();
    };
    btnContainer.appendChild(extraButton);
  }
  ...
  try {
    await setDoc(
      roomRef,
      {
        teams,
        auctionPlayers,
        bidders: connectedBidders, 
        kickedBidders: window.kickedBidders || [], // ✨ 강퇴 명단 저장
...
// ✨ 강퇴 처리 함수
window.kickBidder = async function (nickname, teamId) {
  if (!window.isHost || !nickname || nickname.startsWith("팀장 ")) return;
  ...
  const currentKicked = data.kickedBidders || [];
  
  if (!currentKicked.includes(nickname)) {
    currentKicked.push(nickname);
  }

  // 팀 정보에서 해당 팀장 제거
  const team = teams.find(t => t.id === teamId);
  if (team) {
    team.leader = `팀장 ${team.id}`;
    team.name = "TEAM_이름을 입력해주세요";
  }

  addLog(`[강퇴] 방장이 '${nickname}'님을 강퇴했습니다.`, "system");
  
  window.kickedBidders = currentKicked;
  connectedBidders = connectedBidders.filter(name => name !== nickname);
...
function startSync(enteredCode) {
  ...
  unsubscribeRoom = onSnapshot(roomRef, (snapshot) => {
    ...
    // ✨ 강퇴 여부 확인 (경매자용)
    if (!window.isHost && myNickname && state.kickedBidders && state.kickedBidders.includes(myNickname)) {
      showCustomModal({
        title: '강퇴 알림',
        message: '방장에 의해 강퇴되었습니다. 초기 화면으로 이동합니다.',
        onConfirm: () => {
          location.reload(); // 첫 화면으로 돌아가기
        }
      });
      unsubscribeRoom();
      return;
...
window.editLeaderName = function (teamId) {
  if (!window.isHost) return;
  const team = teams.find((t) => t.id === teamId);
  const isRealUser = team.leader && !team.leader.startsWith("팀장 ");

  showCustomModal({
    type: 'prompt',
    title: '팀장 이름 변경',
    message: isRealUser 
      ? `'${team.leader}' 팀장의 이름을 변경하거나 강퇴할 수 있습니다.`
      : '새로운 팀장 이름을 입력하세요:',
    defaultValue: team.leader,
    extraBtn: isRealUser ? {
      label: '강퇴하기',
      color: '#f39c12',
      onClick: () => window.kickBidder(team.leader, teamId)
    } : null,
    onConfirm: (newName) => {
...
window.addTeam = function () {
  ...
  const setupCountEl = document.getElementById("setup-team-count");
  if (setupCountEl) setupCountEl.textContent = teams.length;
  
  renderTeams();
  saveState();
  addLog(`[시스템] 새로운 팀(${newTeam.leader})이 추가되었습니다.`, "system");
};

// ✨ 마지막 팀 삭제 함수
window.removeLastTeam = function () {
  if (!window.isHost || teams.length === 0) return;
  const lastTeam = teams[teams.length - 1];
  window.deleteTeam(lastTeam.id);
};
...
window.deleteTeam = function (teamId) {
  ...
  showCustomModal({
    type: 'confirm',
    title: '팀 삭제',
    message: `'${team.name || team.leader}' 팀을 삭제하시겠습니까?<br>팀원이 있을 경우 대기 명단으로 자동 복구됩니다.`,
    onConfirm: () => {
      // 팀원들을 대기 명단으로 복구
      if (team.members && team.members.length > 0) {
        team.members.forEach(member => {
          if (member) auctionPlayers.push(member);
        });
      }
      ...
      renderWaitingList();
      saveState();
      addLog(`[시스템] '${team.name || team.leader}' 팀이 삭제되었습니다.`, "system");
    }
  });
};
            ...
            <div class="team-header">
                <div class="team-identity">
                    <span class="team-name">
                        ...
                        ${window.isHost ? `<i class="fa-solid fa-trash-can edit-name-btn" onclick="deleteTeam(${team.id})" style="color: #ff4655; margin-left: 8px;" title="팀 삭제"></i>` : ""}
                    </span>
...
window.undoAuction = async function () {
  ...
  showCustomModal({
    ...
    onConfirm: async () => {
      ...
      currentPrice = 0; // ✨ 되돌리기 시 점수를 0점부터 다시 시작하도록 수정
      highestBidder = null; // ✨ 입찰자 정보 초기화
      ...
      addLog(
        `[되돌리기] ${lastSaleInfo.player.name} 선수의 낙찰이 취소되고 점수가 0pt로 초기화되었습니다.`,
        "system",
      );
...
function updateAuctionDisplay() {
  ...
  if (currentAuctionPlayer) {
    ...
    // ✨ 경매 진행 중이면 랜덤 뽑기 오버레이 숨김
    if (randomPickOverlayEl) randomPickOverlayEl.style.display = "none";
    
    // ✨ 방장이고 경매 대상이 있으면 재추첨 버튼 표시
    if (window.isHost && !isRandomPicking) {
      if (rePickBtn) rePickBtn.style.display = "inline-block";
    }
  } else {
    ...
    // ✨ 방장이고 대기 명단에 선수가 있으면 랜덤 뽑기 중앙 버튼 표시
    if (window.isHost && auctionPlayers.length > 0 && !isRandomPicking) {
      if (randomPickOverlayEl) randomPickOverlayEl.style.display = "flex";
    } else {
      if (randomPickOverlayEl) randomPickOverlayEl.style.display = "none";
    }
  }
...
// ✨ 선수 랜덤 뽑기 함수 (애니메이션 포함)
window.pickRandomPlayer = function () {
  if (!window.isHost || auctionPlayers.length === 0 || isRandomPicking) return;
  if (isPlacingPlayer) {
    showCustomModal({ message: "먼저 낙찰된 선수의 팀 배치를 완료해주세요." });
    return;
  }
  ...
  let count = 0;
  const maxCount = 15; // 15번 섞기 효과
  const interval = setInterval(() => {
    const randomIndex = Math.floor(Math.random() * auctionPlayers.length);
    const tempPlayer = auctionPlayers[randomIndex];

    // 중앙 카드에 임시로 표시하여 애니메이션 효과 제공
    targetPlayerNameEl.textContent = tempPlayer.name;
    targetPlayerRoleEl.innerHTML = `${tempPlayer.role}<br><span style="font-size: 0.8em; opacity: 0.9; color: ${getTierColor(tempPlayer.rank)}; font-weight: bold;">${tempPlayer.rank}</span>`;
    currentPlayerCardContentEl.style.display = "block";
    
    // 입찰 정보 초기화 표시
    currentPriceEl.textContent = "???";
    highestBidderInfoEl.textContent = "추첨 중...";
    ...
    if (count >= maxCount) {
      ...
      // 최종 랜덤 선택
      const finalIndex = Math.floor(Math.random() * auctionPlayers.length);
      const chosenPlayer = auctionPlayers[finalIndex];
      
      addLog(`[랜덤 뽑기] '${chosenPlayer.name}' 선수가 추첨되었습니다.`, "system");
      selectPlayerForAuction(finalIndex);
    }
  }, 100);
};

// ✨ 재추첨 함수 (현재 선택된 선수를 명단으로 복구하고 CLICK 버튼으로 돌아감)
window.rePickRandomPlayer = function () {
  if (!window.isHost || !currentAuctionPlayer || isRandomPicking) return;

  showCustomModal({
    type: 'confirm',
    title: '재추첨',
    message: `'${currentAuctionPlayer.name}' 선수를 대기 명단으로 돌려보내고 다시 추첨하시겠습니까?`,
    onConfirm: () => {
      // 현재 선수를 대기 명단에 다시 추가
      auctionPlayers.push(currentAuctionPlayer);
      ...
      addLog(`[재추첨] 방장이 재추첨을 선택하여 현재 매물이 취소되었습니다.`, "system");
...
// ✨ 대기 명단 마우스 휠로 가로 스크롤 가능하게 처리
if (waitingListEl) {
  waitingListEl.addEventListener("wheel", (e) => {
    if (e.deltaY !== 0) {
      e.preventDefault();
      waitingListEl.scrollLeft += e.deltaY;
    }
  });
}
